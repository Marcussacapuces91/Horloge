 ...
 *
 *  Unless required by Horlogelicable law or agreed to in writing, software
...
static const unsigned I2C_ADDR = 0x70;
...
/**  
 * Méthode Horlogeelée perpétuellement.
 */
  inline
  void loop() {
    ...
    if (sec != oldSec) {
      ...
      alpha4.writeDigitAscii(0, '0' + hh / 10);
      ...
      alpha4.setBrightness( constrain(_BV(sensorValue / 256) * 3, 2, 30) / 2 );
      alpha4.writeDisplay();
    }
...
*/
  ...
  void updateURL(const String& aURL) {
    url = aURL;
    timeClient.setPoolServerName(aURL.c_str());  
//    timeClient.update();
  }

  void updateUTCOffsetInSeconds(const long& aOffset) {
    utcOffsetInSeconds = aOffset;
    timeClient.setTimeOffset(aOffset);
  }
